...
function move() {
    ...
    if (direction == 'x-') {
        if (coord_x === 0) { //удаляем границы поля x-
            coord_x = FIELD_SIZE_Y;
        }
        new_unit = document.getElementsByClassName('cell-' + (coord_y) + '-' + (coord_x - 1))[0];
    }
    else if (direction == 'x+') {
        if (coord_x === FIELD_SIZE_X - 1) { //удалеям границы поля x+
            coord_x = -1;
        }
        new_unit = document.getElementsByClassName('cell-' + (coord_y) + '-' + (coord_x + 1))[0];
    }
    else if (direction == 'y+') { //удалеям границы поля y+
        if (coord_y === 0) {
        ...
        if (coord_y === FIELD_SIZE_Y - 1) { //удалеям границы поля y-
            coord_y = -1;
        }
        new_unit = document.getElementsByClassName('cell-' + (coord_y + 1) + '-' + (coord_x))[0];
    }
 ...
 * @returns {boolean}
 */
function haveFood(unit) { //+ создание бомбы каждые 5 очков
    var check = false;
    ...
    if (unit_classes.includes('food-unit')) {
        ...
        let snakeScore = document.querySelectorAll('.snake-score span')[1]; //онлайн табло счета
...
function finishTheGame() {
    ...
    let snakeScore = document.querySelectorAll('.snake-score span')[1];
    snakeScore.style.color ='red';
}
